...
/*
문제 13. 
--EMPLOYEES 와 DEPARTMENTS 테이블에서 JOB_ID가 SA_MAN인 사원의 LAST_NAME, JOB_ID, 
...
JOIN
    departments d
ON
    tbl.department_id = d.department_id;
...
SELECT
    ...
    NVL((
        SELECT
            TRUNC(AVG (e.salary),0) AS result
        FROM
            employees e
        WHERE
...
SELECT
    d.*,
    loc.street_address, loc.postal_code, NVL(tbl.result, 0) AS 부서별평균급여
FROM
    departments d
JOIN
    locations loc
...
LEFT JOIN
    (
        SELECT
            department_id, TRUNC(AVG (e.salary),0) AS result
        FROM
            employees e
        GROUP BY
            department_id    
...
ON
    d.department_id = tbl.department_id
ORDER BY
    tbl.result;
