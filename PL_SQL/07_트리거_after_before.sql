...
BEGIN
    ...
    -- UPDATING은 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문
    IF UPDATING
        THEN
            v_type := '수정';
    ELSIF DELETING
        THEN
            v_type := '삭제';
    END IF;    
    -- 본격적인 실행 구문 작성 (백업 테이블에 값을 INSERT
    -- -> 원본 테이블에서 UPDATE or DELETE 된 사용자의 정보 및 기타 정보)
    INSERT INTO
        tbl_user_backup
    VALUES
        (:OLD.id, :OLD.name, :OLD.address, sysdate, v_type, USER());
END;
...
CREATE OR REPLACE TRIGGER trg_user_insert
    ...
    FOR EACH ROW
BEGIN
    -- INSERT 예정인 name의 값에서 첫글자만 추출 후 *** 붙이기
...
BEGIN
    ...
    v_total := :NEW.total; -- 주문 수량을 가져옴
    v_product_no := :NEW.product_no; -- 주문 상품의 번호를 가져옴
    ...
    FROM
        product
    WHERE
        product_no = v_product_no;  -- 상품 번호를 통해 재고 수량을 조회

    IF v_product_total <= 0     -- 재고가 없는 경우
        THEN
            RAISE zero_total_exception;
    ELSIF v_total > v_product_total   -- 주문 수량이 재고 수량보다 많은 경우
        THEN
            RAISE quantity_shortage_exception;   
    END IF;

    -- 재고 수량이 넉넉하다면 주문 수량만큼 재고 수량 수정        
    UPDATE
        product
    SET
        total = total - v_total
    ...
    EXCEPTION
        WHEN quantity_shortage_exception
            THEN
...
-- 트리거 내에서 예외가 발생하면 수행중인 INSERT 작업은 중단되며 ROLLBACK이 진행
