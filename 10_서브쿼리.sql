...
-- David라는 사람이 여러 명인데, 그중 가장 적은 급여를 받는 David보다 급여가 높은 사람 조회
SELECT
    *
FROM
    employees
...
-- EXISTS 연산자가 1이 조회될 때 데이터가 존재한다는 것을 판단하여 employees에서 해당 사원의 정보 조회
SELECT
    *
FROM
    employees e
...
-- SELECT 절에 서브쿼리 붙이기
-- 스칼라 서브쿼리 라고도 칭한다
-- 스칼라 서브쿼리 : 실행 결과가 단일 값을 반환하는 서브쿼리. 주로 SELECT절, WHERE절에서 사용
SELECT
    e.first_name,
    d.department_name
FROM
...
: 조회할 컬럼이나 데이터가 대용량인 경우,
해당 데이터가 수정, 삭제 등이 빈번한 경우 (sql 가독성이 조인이 좀 더 뛰어나다)
*/
...
-- salary로 정렬을 진행하면서 바로 ROWNUM을 붙이면 ROWNUM이 정렬되지 않는 상황이 발생.
...
-- ROWNUM을 붙이고 범위를 지정해서 조회하려고 하는데,
-- 범위 지정도 불가능하고, 지목할 수 없는 문제가 발생.
-- 이유: WHERE절 먼저 실행하고 ROWNUM이 SELECT 되기 때문.
-- 해결: ROWNUM까지 붙여 놓고 다시 한 번 자료를 SELECT 해서 범위를 지정.
SELECT
    ROWNUM AS rn, tbl.*
FROM
    (
