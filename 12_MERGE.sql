...
/*
    ...
    한 테이블에 해당하는 데이터가 존재한다면 UPDATE,
    존재하지 않는다면 INSERT로 처리
*/
...
-- MERGE
MERGE INTO
    emps_it a -- merge를 할 타겟 테이블
    USING -- 병합 시킬 데이터 (테이블 이름, 서브쿼리 등..)
        (
            SELECT
                *
            FROM
            ...
            WHERE
                job_id = 'IT_PROG'
        ) b -- 병합하려는 데이터를 서브쿼리로 표현
    ON
       (a.employee_id = b.employee_id) -- employee_id를 통해 양쪽 테이블의 데이터 존재 유무 확인
        
WHEN MATCHED THEN -- 조건이 일치하는 경우 (양쪽 테이블에 모두 데이터가 있는 경우 -> UPDATE)
    UPDATE
    SET
...
WHEN NOT MATCHED THEN -- 조건이 일치하지 않는 경우 (한쪽 테이블에만 데이터가 있는 경우 -> INSERT
    INSERT
    VALUES
...
INSERT INTO
    ...
    (employee_id, first_name, last_name, email, hire_date, job_id)
VALUES
    (102, '렉스', '박', 'LEXPARK', '01/04/06', 'AD_VP');

INSERT INTO
    emps_it
    (employee_id, first_name, last_name, email, hire_date, job_id)
VALUES
    (101, '니나', '최', 'NINA', '20/04/06', 'AD_VP');

INSERT INTO
    emps_it
    (employee_id, first_name, last_name, email, hire_date, job_id)
VALUES
    (103, '흥민', '손', 'HMSON', '20/04/06', 'AD_VP');
