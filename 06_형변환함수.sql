...
-- 날짜를 문자로 형 변환할 때 원하는 형식으로 날짜를 표현할 수 있다.
SELECT
...
-- 서식 문자와 함께 사용하고 싶은 문자를 ""로 감싸서 전달
SELECT
...
SELECT
    TO_CHAR(20000, 'L99,999') -- 해당 지역의 화폐 단위 자동 입력
FROM
    dual;
    

SELECT
    TO_CHAR(20000.29, '99999.9') -- 반올림 지원
FROM
    dual;
...
SELECT
    '2000' + 2000   -- 자동 형 변환 (문자 -> 숫자)
FROM
    dual;
    
    
SELECT
    TO_NUMBER('2000') + 2000    -- 명시적 형 변환
FROM
    dual;
    
    
SELECT
    '$3,300' + 2000 -- 에러 -> 형 변환이 필요한 상황
FROM
    dual;
...
-- 주어진 문자열을 모두 변환해야한다. 일부만 변환하면 에러
SELECT
...
SELECT
    TO_DATE('2024년 02월 12일', 'YYYY"년" MM"월" DD"일"')
FROM
    dual;
    
    
-- xxxx년 xx월 xx일 -> 문자열 형식으로 변환
-- 조회 컬럼명은 dateInfo, dual에서 조회
SELECT
    TO_CHAR(
        TO_DATE('20050102'), 'YYYY"년" MM"월" DD"일'
    ) AS dateInfo
FROM
    dual;
    
    
-- NULL 형태를 반환하는 함수 NVL(컬럼, 변환 할 타겟값)
SELECT
    first_name,
...
/*
문제 1.
현재일자를 기준으로 employees테이블의 입사일자(hire_date)를 참조해서
근속년수가 17년 이상인 사원을 다음과 같은 형태의 결과를 출력하도록 쿼리를 작성해 보세요. 
...
FROM
    employees
WHERE
    TRUNC((sysdate - hire_date) / 365) >= 17
ORDER BY
    근속년수 DESC;
